...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        if "save_best" in train_cfg:
            self.save_best = train_cfg["save_best"]
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        for it in range(start_iter, tot_iter):
            ...
            if self.log_dir is not None and len(rewbuffer) > 0:
                ...
                if self.save_best:
                    current_mean_reward = np.mean(rewbuffer)
                    if current_mean_reward > self.best_mean_reward * (1 + improvement_threshold):
                        self.best_mean_reward = current_mean_reward
                        best_model_path = os.path.join(self.log_dir, "best_model.pt")
                        self.save(best_model_path)
                        # print(f"New best model saved with mean reward: {current_mean_reward:.3f}")
                # Log information
                self.log(locals())
                # Save model
